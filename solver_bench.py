...
实测（本机，官方 vortexShed 同款通道域阶梯 tet 网格）：
  ...
  | 网格 | 未知量 | 直接 LU | 仅 AMG | **当前默认（v3）** |
  | --- | --- | --- | --- | --- |
  | h=D/8, T=0.125D | 48,840 | （第三轮 5.77 s/步） | — | **min 1.29 / median 1.30 s/步** |
  | h=D/8, T=0.25D | 97,680 | 32.64 s/步 | 12.56 s/步 | **min 1.30 / median 1.32 s/步（达标 <2 s）** |
  | h=D/8, T=0.5D | 195,360 | （直接 LU 不可行） | — | **min 3.10 / median 3.12 s/步** |

  v3 = 组装三元组向量化（numpy 块 + 一次 concatenate）
     + 动量走 `_bicgstab_ladder`：**无预条件 BiCGSTAB 0.04-0.06 s**（Jacobi 同级、ILU(0) 要 0.73-0.76 s
       —— 预条件是净亏 18×），每级都过残差校验后才采用
     + 压力走 AMG(max_coarse=200) 预条件 CG；残差与直接 LU 逐位一致。
