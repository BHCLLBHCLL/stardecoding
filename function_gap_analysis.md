...
2. **二进制编码状态表的数值流**（改进④部分完成）：id=3B 大端(=×256)/flags/version/
   ...
   **R3 部分达成（2026-09-13）**：T 载荷**只存在于二进制文件**（ASCII 的 T 只有 banner）；
   容器元素 8B（81/82 + u32 count + u16 id，331 个中 155 个 id 解析入对象图）；
   ...
   **仍开放**：T 载荷是字节流（元素长度可为奇数），**已解码 15.84%（manifold）/4.98%（airfoil，仅容器）/0%（vibratingPipe）**，
...
| 求解状态/解数据 | ✅ **S3 根节点变体**（`extract_solution_fields` 支持 `FvRepresentation` 根：官方新保存件无 `SolutionRepresentation`，实测官方桥生成翼型件 21 字段/16987 单元抽取成功）；✅ **R4 官方解差分**（vortexShed 官方参考量：D=0.04/Re=200/St=0.1752/振幅 0.2805/Continuity 3.2e-10 + 同工况环形域几何 + 误差带判定；场级逐点差分未做）；✅ 已抽取 .sim 内嵌解场（G5：SolutionRepresentation → FvRegion cells DUP 组 map，标量 n==CellCount / 矢量 x3；CLI `--solution-fields`/`--solution-csv`，GUI 真解场标量着色） | .simh HDF5 路径待语料扩充（本机语料无 .simh 文件；h5py 未装） |
...
> ~~查看器「打开-浏览-看图」约 65%；编辑器约 18%~~ → **F0–F8 后重估：查看器约 95%、编辑器约 80%（其中可重开复现约 72%）**（详见 `star_gui_next.md` §1.1）。
> 编辑器已闭环：已有对象标量/颜色/透明/Keys/CurrentView（F1）、新对象插入（复制/新建场景/显示器，F2）、真 STL 导入 + Float8 顶点回写（F3）；
> 场景/Vis/Parts 过滤器/Representation/框选测距（F4）、边界↔FaceTypes 精确高亮（F5）、监视器/残差曲线 + 标量着色（F6）已接线；网格生成/求解运行/Post 20 动作随 C/N/P/V 波启用。
> 剩余 needs_kernel：Mesh>清除/转 2D、3D-CAD 界内 B-Rep 建模；解析侧剩余缺口（T 块、数组字段、边界→面片、体网格）不变。
