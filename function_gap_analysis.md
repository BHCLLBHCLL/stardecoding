...
## 6.5 查看器交付（star_gui，另见 star_gui_design.md / star_gui_plan.md）

| 里程碑 | 状态 |
| --- | --- |
| M0 主窗口骨架+打开+摘要 | 完成 |
| M1 仿真树+属性面板 | 完成 |
| M2 3D 网格显示（extract_mesh→VTK） | 完成 |
| M3 场景/显示器/视图相机 | 完成 |
| M4 导出+--cli | 完成 |
| M5 Part 显隐/Region 高亮/边线 | 完成 |
| M6 主题/i18n/测试/文档 | 完成（21 GUI 测试绿） |

> 第 5 节"重建/导出"能力的查看器侧已交付；解析侧剩余缺口（T 块文法、数组表字段语义、边界→面片映射）不变，见本文件相应章节。
