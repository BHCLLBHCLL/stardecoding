...
| 后处理>（Post，绘图之后） | view（`star_gui_postprocess.py` 纯逻辑桥 + `star_gui.py` `cmd_post_action`：19 个 `Post>*` 动作 → 视口标量着色（`_render_post_color`）/ 等值面·剖面·裁剪·阈值·镜像三角面（`_render_post_geometry` → `Star3DViewport.add_actors`）/ XY·直方图曲线（`_render_post_series` → `PlotPane.canvas.set_series`）；场来源 `resolve_fv` 三路径 + 默认求解器节点场补入；无 FVM/无场诚实降级） |
...
| 打包 / 版本发布（X4） | view（`star_gui_package.py`：`toolchain()`/`available_packager()` 探测 PyInstaller/cx_Freeze/nuitka → 本环境全缺失则 `build_plan()` 诚实降级（`available=False` + `pip install pyinstaller` 指引，不假装打包）；`resource_manifest()`/`missing_resources()` 运行时资源清单（theme qss + javadoc 目录 + 用户指南 + 对标文件 + README）；`read_requirements()` 解析运行依赖；`local_modules()` 扫描模块级/惰性 import 供 `--hidden-import`；`version_manifest()`/`version_string()` 复用 `star_gui_help` 的 `APP_NAME`/`APP_VERSION`；`pyinstaller_command()`/`pyinstaller_spec_text()` 产出 `.spec`+命令行；`inno_setup_text()` 产出 Inno Setup 安装器 `.iss`；`build_script_text()`/`release_notes()` + `write_build_scripts(outdir)` 一键落盘 `.spec`/`.iss`/`build_star_gui.py`/`RELEASE_NOTES.md` 四件套） |
...
| 后处理：标量/矢量 color-by（V1） | view（GUI `Post>ColorBy` → 视口着色（`star_gui_postprocess.py`+`cmd_post_action`）；内核 `postprocess.py` 色彩映射内核：`DEFAULT_COLORMAP_VALUES` 官方风格蓝→黄→红 4n 断点 + `parse_colormap`（位置降序自动翻转；断点 <8 或非 4 倍数返 `(None,None,None)`）+ `sample_colormap` 重采样 `(n,4)` RGBA + `scalar_range`（含 robust 百分位）+ `map_scalars` 标量→RGBA + `vector_magnitude`（1D 直通）+ `color_by`（scalar/magnitude/component 统一入口，非 2D 场取分量诚实 `ValueError`）；纯 numpy，与 G8 `extract_scene_display` 官方色表断点同源） |
| 后处理：显示器全家桶（V2） | view（GUI `Post>IsoSurface/Section/Clip/Threshold/Mirror` → 视口三角面（`_render_post_geometry`）；`Post>Glyphs` → 视口矢量符号线元（`_render_post_glyphs` → `glyph_lines_polydata`（points→tips N 条 VTK_LINE）/`glyph_actor`（按幅值着色），载荷箭头 tip/vector/scale）；内核 `postprocess.py`：`marching_tets` 逐四面体等值面（法向按标量梯度定向）+ `section_plane`/`clip_plane`（`keep="negative"` 保 `(x−p)·n≤0`）+ `extract_surface`（剔内部共享面）+ `threshold_cells`（非单元场诚实 `ValueError`）/`threshold_surface` + `mirror_geometry`（`T[:, ::-1]` 反绕序+merge）+ `vector_glyphs`（stride 抽稀+箭头）+ `streamline`/`streamlines`/`pathline`/`particle_trace`（RK 积分，`_safe_norm` 纯 numpy）；cube_tet_mesh(3) 等值面 1.0/剖面 1.0/半盒裁剪 4.0/全盒外表面 6.0 精确断言） |
| 后处理：派生零件（V3） | view（GUI `Post>Probe/Line/PlaneSample/IsoVolume` 载荷（`run_action`）；内核 `postprocess.py`：`locate_cell` 体积坐标重心定位（域外 `(-1,None)`）/`sample_scalar`/`sample_vector`/`cell_to_vertex` + `probe`（points/cells/inside/values）/`line_sample`（+t/length）/`plane_sample`（+u/v/grid_shape/normal/point）/`iso_volume`（等值体积 fraction，接受逐顶点场按单元顶点均值折算）/`threshold_part`（+cells/count/volume/lo/hi）/`plane_box_polygon`/`DerivedCache`（`get_or_compute` 命中未命中计数 + key/clear/`__len__`）） |
| 后处理：绘图数据（V4） | view（GUI `Post>XY/Histogram` → `PlotPane.canvas.set_series` 折线（`_render_post_series`）；内核 `postprocess.py`：`xy_series` + `histogram`（counts/edges/centers/bin_width/n/range/mean/density/[pdf]）+ `cumulative_distribution`（x/cdf/n）+ `decimate_series`（x/y/n/original）+ `MonitorBuffer` 容量滚动缓冲（append/series/clear）；供 `star_gui_plots.py` 渲染，数据面与官方曲线同源） |
| 后处理：注记/图例/色标尺/动画（V5） | view（GUI `Post>Colorbar/Legend/Animate`；内核 `postprocess.py`：`colorbar_ticks`/`colorbar_strip` 色标尺几何 + `legend_items` + `annotation` 注记链 + `frame_times`/`frame_indices`/`frame_name`（补零命名 `frame_0007.png`）+ `export_animation` PNG 序列/GIF 真写，mp4 无 ffmpeg 诚实降级 `"unavailable(ffmpeg)"`；动画帧序正确） |
| 后处理：数据写出 CSV/EnSight/CGNS（V6） | view（GUI `Post>ExportCSV/ExportEnSight/ExportCGNS`；内核 `postprocess.py`：`write_csv`/`export_csv`（含 cell/x/y/z 几何列）+ `write_ensight`（`.case`/`.geo`/`.dat` 三件套）+ `write_cgns`（CGNS SIDS HDF5，C=cells+1 1-based；无 h5py 诚实 `RuntimeError`）；**门面/工厂** `_solver_fv`（试 `fv`/`fvm`/`_fv`）+ `fields_from_solver`（velocity/speed/pressure/rho/T/mach）+ `PostProcessor`（color/streamline(s)/isosurface/section/clip/threshold/mirror/glyphs/derived/probe/line/plane/iso_volume/xy/histogram/cdf/colorbar/legend/animate/export_csv/export_ensight/export_cgns + `from_solver`）+ `make_postprocessor`（8 别名，未知 `ValueError`）；`tests/test_postprocess.py` 49 项全绿 + `self_test.py` V 波锚点，occ 环境全量 pytest 560 passed 27 skipped 无回归） |
