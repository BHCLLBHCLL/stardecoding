...
| 里程碑 | 内容 | 产出 | 状态 |
| --- | --- | --- | --- |
| M0 | 骨架：主窗口 + 打开文件 + 摘要 | star_gui.py 可运行 | 未开始 |
| M1 | 仿真树 + 属性面板 + 层报告 | 左栏可用 | 未开始 |
| M2 | 3D 网格显示（核心） | 面网格 + 交互 | 未开始 |
| M3 | 场景/显示器/视图 + 颜色 | 场景标签页 | 未开始 |
| M4 | 消息/进度/状态栏 + 导出 + CLI 模式 | 完整窗口 | 未开始 |
| M5 | CAD 几何显示（渐进）+ 切片/边界着色 | 几何增强 | 未开始 |
| M6 | 打磨：主题/图标/i18n/测试/打包 | 发布候选 | 未开始 |
