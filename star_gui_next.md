...
Save 能改**已有对象图行**。作为查看器，从「打开-浏览-看图」的约一半，提高到大约 ~~65%~~（E0–E8 时点，**已被 §1.1 取代**）。
作为编辑器，从接近 0 提高到大约 ~~18%~~（同被 §1.1 取代）——当时真正闭环的只有「改已落盘对象的标量/列表字段」。
...
## 1.1 F0–F8 后重估（判定以本节为准）

F0–F8 及 C/N/P/V/X/A 波接线后，上节 E0–E8 时点的 **65% / 18%** 已过时：

| 项 | E0–E8 时点 | F0–F8 后重估 | 依据 |
| --- | --- | --- | --- |
| 查看器（打开-浏览-看图） | ~65% | **~95%** | F5 边界↔FaceTypes 精确高亮/拾取、F6 监视器与残差曲线重建 + 标量着色、按 Part/场景 STL、体网格线框 |
| 编辑器（改-存-重开复现） | ~18% | **~80%**（可重开复现 ~72%） | F1 已有对象标量/颜色/透明/Keys/CurrentView 落盘闭环、F2 新对象插入（复制/新建场景/显示器）、F3 真 STL 导入 + Float8 顶点回写、F4 场景/Vis/Parts 过滤器/Representation/框选测距 |
| 求解/连接 | 灰显（正确禁用） | Run 启用、Pause/Step/Stop 按态启停；Server 仍诚实禁用 | P10 `_sync_solution_actions`，`test_gui_editor.py` 锁初态 |

剩余 needs_kernel（不假装）：Mesh>清除/转 2D、3D-CAD 界内 B-Rep 建模（草图/拉伸/缝边）、Parasolid 写（B 路线）。
...
| 求解/连接 | 启用 | Run 实/其余按态 | P10 `_sync_solution_actions`；Server 仍 `setEnabled(False)` | 见 §1.1 |

**合计（E0–E8 时点，已被 §1.1 取代）**：查看器 **~65%**；编辑器 **~18%**（可重开复现约 **8%**）。

`star_gui_parity.md` 已在 F0 按审计对齐（导入为真 STL、变换为顶点回写、新建显示器为 persist）。E0–E8 时点口径以本文件 §2 为准；**F0–F8 后以 §1.1 为准**。
