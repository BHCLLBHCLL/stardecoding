...
| G8 | 场景显示参数解码 | Displayer/颜色映射/图例/注记属性 | 场景按官方参数渲染 |
| G9 | 二进制状态表完整文法（写侧前置） | G1 配对法推广到 binary 变体 | binary 记录流可逆 |
...
| W6 | 差分回归自动化 | `try_official_resave()` + `compare_object_graph()` 进 tests（有许可触发）；无许可走结构自检+重读一致 |
