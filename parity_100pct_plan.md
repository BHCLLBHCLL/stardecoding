...
| C5 | 表面包裹 wrapper（收缩包裹+特征捕捉+局部尺寸） ✅ 2026-09-04 | A（自研） | **达成（`occ_wrap.py` 纯 numpy，两环境皆真跑）**：收缩包裹——`shrink_wrap`（精确 triangle-AABB Akenine-Möller 相交判定 占位 → 边界面 slab-投影覆盖提取）：开底方孔/开口筒等**不闭合输入**也密封成**必然水密**闭合表面（`boundary_edges` 0 边界边），可导出 STL 往返面数一致；自适应局部尺寸——`wrap_mesh`（先出水密均匀包裹 + refine 球区/features 特征点附近的**质心扇形加密**：只切三角内部、不拆共享边，故构造性保持水密）：refine/特征区面密度增高、整体仍水密；特征捕捉——`detect_features`（相邻面二面角>阈值判 sharp 边，阈值越大召回越少，返回 (a,b,dihedral) 三元组）；`tests/test_occ_wrap.py` **7 用例两环境（默认 3.14 / conda occ）全真跑全绿**；C1–C4 基线与 self_test/run_all（默认 104 过+26 skip、occ 130 全过）/batch_parse 21 无回归 | 收缩包裹水密、可局部加密、可捕捉特征（包裹→加密→特征 达成） |
...
| N2 | 表面 remesher（Delaunay/前沿推进、曲率自适应尺寸场） ✅ 2026-09-04 | A+B | 教程表面网格尺寸分布对标 —— **达成**：A)一致性边中点均匀细分 `refine_surface`（每级每共享边只拆一次 → 每三角 4 个，水密保持，多级 4^n 面）；B)曲率自适应尺寸场重网格化 `mesh_remesh.py`（N2b）：`curvature_field` cotan 平均曲率幅值（平面内点≈0、球面中纬≈2 且交差<0.1）、`size_field` 单调 `L_v=target_max/(1+κ_v/κ60)`（高曲率→更短边）、`remesh_surface` 局部算子 split/collapse/flip/smooth（边界边不劈分、翻边洛链校验、collapse 防非流形、末端顶点压缩），闭环输入保持水密(欧拉=2)、开面边界半环数不变。验收 `tests/test_mesh_remesh.py` 8 项全绿（曲率对标/尺寸单调/波峰边<干谷 0.85×、cube 水密欧拉 2、球面无损、开面边界保持、平坦不加密）。2026-09-05 硬化：split/collapse 边处理顺序改为**显式全序（边长,边索引）**，消除对称网格等长边 `np.argsort` 决胜在 numpy 1.x/2.x 的差异 → 两环境重网格化输出逐位一致（nV/nF/边长均同）；曲率驱动验收改 `max_iter=4`（确定性波峰/干谷边比 0.814<0.85，margin≈4%）。C)Delaunay/前沿推进 `mesh_front.py`（N2c）：`bowyer_watson` 2D 增量 Delaunay（空外接圆违规=0）、`delaunay_metric`+`delaunay_flip` 内蕴角准则 α+β≤π 逐内边翻转（非 Delaunay 凸四环翻转后违规归零、边界保持）、`advancing_front` 边界环细分→理想雀形推进+关隙复用+质心遮挡防重（域覆盖面积误差<5%、全正面积、边界保持、带内孔环域覆盖≈0.96）、`remesh_wavy_advancing` 图曲面按 z=f(x,y) 提升。验收 `tests/test_mesh_front.py` 12 项全绿 |
| N3 | 体积网格器三路线：tet(Delaunay)→Gmsh/netgen 桥；poly(tet 的 Voronoi 对偶)；trimmer(八叉树切割,自研排最后) ✅ 2026-09-05 | A+B | adjointWing 单元数同量级 —— **tet 双路由达成**：`mesh_tet.py` `tet_mesh` 对水密闭合表面自动选路由——A)scipy.spatial.Delaunay（表面顶点+内部均匀栅格填充点，even-odd 射线法 `point_in_mesh`（Möller–Trumbore）判内，负体积裁剪）；B)Gmsh（面环→体、共享边去重 `edge_line`、Frontal-Delaunay 3D）。`tet_quality` 纯 numpy：逐四面体体积/负单元/6 边纵横比。非闭合输入抛 ValueError。对照面 cube 走通全部正体积。**N3b 达成（2026-09-05）**：C)`mesh_poly.py` poly = tet Voronoi 对偶——`clip_convex` Sutherland-Hodgman 3D 凸多面体平面裁剪（两类退化接触皆治：平面恰过既有顶点时收集保留面贴面顶点补全截面多边形，否则盖帽缺失体不闭；平面与既有面重合时跳过重复盖帽，否则该面贡献重复计）、`poly_volume` 散度定理面扇形体积、`voronoi_dual` 每顶点自包围盒被全部 Delaunay 边邻接中垂面裁剪（内部点=精确 Voronoi 胞；clipped 全体单元无重叠铺满包围盒——均匀栅格内部胞体积≈h³ 且 6 面、体积和==盒体积不变量）、`poly_mesh` 水密表面→tet→对偶完整工作流（立方体 poly 体积和≈1.0）；D)`mesh_trimmer.py` 八叉树 trimmer——表面相交叶（Akenine-Möller，复用 C5 `_tri_box_overlap`）逐级一分八、候选三角沿树下传（子盒⊂父盒只复验父候选）、无接触叶叶心 even-odd 精确分类（内=六面体/外=丢弃）、最深切割叶自六面盒被邻近三角**内侧半空间**逐面裁剪成贴体多面体（凸表面精确贴体：立方体体积守恒=1.0、焊接 UV 球≈4π/3@5%、近表面叶=目标尺寸+球心粗叶分级）。GUI `Mesh>GeneratePoly/GenerateTrimmer` 已接线（`cmd_generate_poly_mesh`/`cmd_generate_trimmer_mesh`，水密守门+HEADLESS 安全+异常不崩）。验收 `tests/test_mesh_poly.py` 9 项 + `tests/test_mesh_trimmer.py` 8 项 + `tests/test_mesh_gui.py` 扩至 9 项全绿 |
| N4 | prism 边界层（推进+碰撞处理） ✅ 2026-09-05 | A | y+/层数达标 —— **达成（`mesh_prism.py` 纯 numpy，两环境皆真跑）**：`vertex_normals` 面积加权顶点法向（与面片绕向一致，不预设内外向）；`ray_hit_distance` Möller–Trumbore 最近三角命中距离（barycentric 边界容差、t>eps 跳过出发面共点命中）；`prism_layers` 推进主入口——层厚几何级数（首层 t0、增长率 r，累计 d_k=t0·(r^k−1)/(r−1)）、每顶点沿推进方向射线最近表面距离 h_v → 允许深度=gap_fraction·h_v（≤0.5 时两侧边界层在通道内构造性不重叠）、面层高 L_f=min(三顶点层高)（提升点按顶点共享→邻面列自动保形一致），超限自动降层（碰撞处理）、极端窄通道塌缩为 0；side="inward"/"outward" 闭合表面自动定向（`_probe_inward` 取面自身绕向法向、质心 c±εn 双探针 even-odd 一内一外判定；开放表面抛 ValueError 改用 "+n"/"-n" 显式定向）；target_faces 局部出层（N5 尺寸控制复用）。`prism_quality` 有向体积（棋盘三分四面体分解）→ 翻转数（符号少数派）/符号一致/体积统计；`first_layer_thickness` 首层厚度均值（y+ 控制旋钮：t0 直接决定首层单元高度，平壁首层质心高=t0/2 精确）。附带修复 `mesh_tet.point_in_mesh` 两处缺陷：① 表面判定对全部三角前置且平面距离+重心双检（旧版只查重心会把面外侧近处点误判在面上）；② 命中按射线参数 t 去重——射线恰好打在共享棱/顶点时相邻三角同点重复计数会把奇偶翻错（轴对齐盒 x=1 面对角棱系统性命中：曾致立方体内点误判在外、trimmer 立方体积守恒跌至 0.9375），去重后体积守恒恢复精确 1.0。验收 `tests/test_mesh_prism.py` 10 项：平板全层精确（层数满/几何级数累计层厚/体积=面积×总厚/首层质心高=t0/2）、立方体满层（side="inward" 自动定向、12 面全满层、推进点全留体内）、窄通道碰撞三分支（gap_fraction=0.9 超比例自动降为 2 层且提升点留体内、=0.5 两侧提升点严格分离无重叠、=0.01 全塌缩 ok=False）、质量指标（翻转 0/符号一致/独立复算体积和一致）、空输入与开放表面自动定向 ValueError 守门；双环境全绿，trimmer/poly/quality 回归修复 |
| N5 | 自定义控制（per part/surface/vertex 尺寸）、thin/directed/extruder ✅ 2026-09-05 | A+B | checkValve 类多控制算例 —— **达成（`mesh_control.py`/`mesh_thin.py`/`mesh_extrude.py` 纯 numpy，两环境皆真跑）**：`mesh_control.resolve_size_field` 多源 Dijkstra 尺寸场——`surface_sizes`/`vertex_sizes`/`region_sizes` 三种来源统一种入（面/顶点/球区域），边权=growth×边长，`L_v=min(base, min_u(s_u+growth·d_geo))` 单调（growth>0 守门），全局 `size_min`/`size_max` 终夹（面级 min/max 先夹 target）；`remesh_with_controls` 把 sizes 透传 `mesh_remesh.remesh_surface` 外部尺寸模式（逐顶点目标边长绕过曲率场，长度须=输入顶点数，_split_pass 返回 L 随分裂演化）。`mesh_prism` 重构 `ray_hit_nearest`(距离,面索引，同距取最小面索引→确定性配对) + `_side_sign`(side→绕向法向符号，inward/outward 探针自动定向、开放表面抛 ValueError) 供 thin 复用。`mesh_thin.thin_mesh` 面级薄判定（面质心沿±推进方向射线命中远处表面→判定薄源面+配对面，侧向棱/背面走向核、法向净纯化、配对去重 f<t_f）、n_sources/n_cells/volume_total 统计。`mesh_extrude` 线性（n_cells=len(F)×n_layers，边保持）/旋转（绕轴旋转扇，Pappus V=A·θ·d——跨 z 方形径向面 2π@16 层≈π、π/2@8 层≈π/4，层间角度均分，`_rotate_about` Rodrigues 旋转纯轴无关）。验收 `tests/test_mesh_control.py` 8 项（面/顶点/区域/多源取小/确定性逐位一致/grid 局部加密近远边比 0.14<0.5 且边界 40 保持）+ `tests/test_mesh_thin.py` 6 项（薄盒体积/底顶薄面/厚立方无薄面/球壳零翻转、n_cells 在 quality 内）+ `tests/test_mesh_extrude.py` 8 项（线性层厚/旋转 Pappus 5%+2%/闭环体）全绿，N2/N3/N4 回归不破 |
| N6 | 质量诊断/统计/修复 + interface 处理 + AMR（运行时） ✅ 2026-09-05 | A | **达成（`mesh_quality.py`/`mesh_interface.py`/`mesh_amr.py` 纯 numpy，两环境皆真跑）**：统一质量度量 `cell_metric` 跨单元类型（tet[M,4]/prism[M,6]/poly dict 列/trimmer hex+cut dict）归一化 0–1——tet 用 SIGMA=12·(3V)^{2/3}/Σℓ²（正四面体=1），poly/hex 用 Wadell 球度 (π^{1/3}(6V)^{2/3})/A（立方≈0.806）；`quality_histogram` 逐单元质量分箱直方图 + 达标判据（n_cells>0 且无退化单元且 mean≥quality_threshold）；`quality_report`/`cell_quality` 统一入口分派 `_native_quality`（复用 `tet_quality`/`prism_quality`/`poly_quality`/`trimmer_quality` 偏差/负单元/体积统计）+ histogram；`repair_cells` 修复（负体积四面体交换两顶点翻转向、退化单元删除）。interface——`interface_faces` 归一化键（质心+量化排序顶点）匹配区域间重合面（只计跨区对，matched=每区每面恰配一次）；`periodic_pairs` 按平移矢量配对周期面片。AMR 运行时——`amr_marks` 质量/百分位驱动标记、`refine_tets` 1:8 红细化（4 角+4 中八面体、共享边中点去重缓存、统计 hanging-node、非一致细化安全）、`register_amr_hook`/`run_amr` 回调注册（P10 监视运行环/P6 湍流联调预留）。验收 `tests/test_mesh_quality_n6.py` **18 项全绿**（occ 真跑；scdm 16 过+2 scipy 门控 skip），N1–N5 回归不破 | 质量 histogram 达标；AMR 在 P 波联调（钩子就绪，P10/P6 接入） |

验收统一走新增 `tests/test_mesh_quality_n6.py`（histogram 达标/repair/interface/AMR 基础），N1–N5 的 `tests/test_mesh_quality.py`（面数/单元数/歪斜度阈值）基线不回退。
...
| P2 | 场函数表达式求值器 ✅ 2026-09-06 | 数学/矢量/逻辑 + 插值器；与官方语法对齐 —— **达成**：新增 `field_fn.py`（`Token`/`_Lexer`/`_Parser`/`_AST`/`EvalContext`/`FieldFunction`/`compile_expression`/`numeric` 词法-语法-求值全链），与官方语法对齐：算术 `+ - * / %`、逻辑 `== != > < >= <= && || !`、嵌套三元 `? :`、数学函数（sin/cos/tan/asin/acos/atan/atan2/sinh/cosh/tanh/ceil/exp/abs/floor/log/log10/sqrt/pow/fmod/mod/min/max/clamp）、矢量（mag/mag2/unit/dot/cross、`[i]` 下标、`.x/.y/.z` 分量、`Position[0/1/2]`）、张量（eigValue/eigVector/trace/norm + `$$$A.eigValue(i)`/`.mag()`/`.mag2()` 点方法）、变量引用（`${Name}` 标量/`$$Name` 矢量/`$$$Name` 张量、`$${Position}`/`$${Time}`）、矢量字面量 `[a,b,c]`、插值器（`interpolateTable(@Table("name"), "column", SPLINE|LINEAR, "units", ${Position}[i])`）、惰性 `alternateValue`/`altValue` 多参（取首个可求值且有限者）；依赖网格/几何（insidePart/distanceToPart/curl/div/grad）诚实 `NotImplementedError`，`compile_expression` 编译预检 + 错误路径诚实拒绝（`/0`/`$$v[3]` 越界/未知函数/未定义变量/语法残句）。`tests/test_field_fn.py` 39 项全绿（官方风格复合/运算符/数学/矢量/张量/插值/嵌套三元/交替值/变量引用/编译预检/诚实拒绝）；`self_test.py` P2 锚点断言 + 诚实拒绝报告全过；occ+scdm 两环境 `self_test`+`batch_parse` 21 文件+`run_all` 36 文件全绿无回归 |
| P3 | 初始化器（field function/常量/表格初值） ✅ 2026-09-06 | Run 前 Initialize 可用 —— **达成**：新增 `init_solver.py` `Initializer` 链式声明（`add_constant`/`add_table`/`add_table_obj`/`add_function`/`declare`）→ `compile()` 预检（source_field 登记 + 函数语法预检）→ `value()` 单点求值（常量直返 / 函数按 `${Position}`/`${Time}`）→ `field()` 批量求值（vector auto/mag）→ `apply_initial()` Run 前对 `solver.vertices` 求值写初场；接入 `solver_run.py`：`DemoDiffusionSolver`/`SolverBackend` 注入 initializer，`set_initializer`/`_set_initial_field`/`_initialize_field` 走初始化器生成初场（缺省回落零场+源边界值），`run_loop` 循环起始注入；错误路径诚实拒绝（source_field 缺失 / 未知函数在求值期 / 语法错误 / 初场维度不匹配）；`tests/test_init_solver.py` 16 项全绿（常量/表格 LINEAR 插值/场函数初场 T=T0+100x/apply_initial demo solver/backend initialize+run_loop 注入/诚实拒绝/维度守门）；`self_test.py` P3 锚点断言+诚实拒绝报告全过；occ+scdm 两环境 `self_test`+`batch_parse` 21 文件+`run_all` 37 文件全绿无回归 |
| P4 | FVM 离散核心：梯度(Gauss/LSQ)、限制器、通量格式 ✅ 2026-09-06 | numpy 向量化 —— **达成**：新增 `fvm_core.py`（`FVM` 四面体单元中心面拓扑：owner/neighbor/面面积/面面积向量/质心/体积；`grad_gauss` Green-Gauss / `grad_lsq` 最小二乘（三维 4×4 cofactor 显式求解，规避 BLAS det 崩溃）、`face_value` 面插值（中心差分+上风格式）、`limiter`/`_barth_ratio` Barth-Jespersen TVD 限制器（gmin/gmax 界限）、`diffusion_flux` 中心差分扩散通量（γ 电导率）、`convection_flux_upwind`/`convection_flux_central` 对流通量、`cube_tet_mesh` 网格构造），全部 numpy 向量化（scdm 无 scipy 亦可运行，occ 可选 scipy.sparse 路径）；接入 `solver_run.py` —— 新增 `FvmDiffusionSolver`（单元中心场，节点初场四顶点均值映射，dt 稳定时间步 `0.8/max(Σ γA/d/V)`，显式前向欧拉，`SolverBackend` 状态机可直接驱动）；`tests/test_fvm_core.py` 26 项全绿（梯度线性场精确/限制器 bounds/通量守恒/收敛/错误路径）；`test_solver_run.py` 32 项（含 6 项 FVM 集成测试）；`self_test.py` P4 锚点断言全过；occ+scdm 两环境 `self_test`+`batch_parse` 21 文件+`run_all` 38 文件全绿无回归 |
| P5 | 压力基求解器：SIMPLE/PISO + Coupled；AMG(pyamg)/ILU ✅ 2026-09-06 | 残差下降曲线健康 —— **达成**：新增 `pressure_solver.py`（`PressureSolver` 基于 P4 `FVM` 单元中心面拓扑的**速度-压力耦合**稳态不可压 SIMPLE 分离求解器）：稀疏线性 `solve_linear` COO(row,col,data) 双路径 —— occ 走 scipy（CSR + `pyamg` smoothed_aggregation AMG，否则 `spilu`+`bicgstab` ILU 加速 / `spsolve` 兜底），scdm 回退**纯 numpy Gauss-Seidel/SOR**（对角占优收敛可靠），RHS 维度不匹配诚实拒绝；SIMPLE 外部迭代：3 动量预测（对流一阶上风/扩散中心差分/压力梯度源隐式求解）→ Rhie-Chow 动量插值面质量通量（正确符号 `d_f[(∇p)_interp·n − (p_N − p_O)/d_n]` 消除并列网格棋盘压力）→ 压力修正泊松 `p'` → 校正 `p += α_p p'`/`u -= d∇p'` + 直接面通量修正 `mdot_f += γ(p'_O − p'_N)`（与压力修正矩阵同 gamma，正是残差收敛关键）→ `_fix_boundary_mdot` 缩放出口通量保 `sum(mdot)=0` 全局质量守恒；欠松弛 α_u=0.7/α_p=0.3；BC（速度 Dirichlet 入口/无滑移壁 + 出口零梯度；压力 Neumann 壁/入口/出口 + 出口压力参考固定 p'=0）；残差=归一化连续性质量不平衡，收敛→0 曲线健康；纯 numpy 约束范数走 `solver_run._safe_norm`（规避 occ np.linalg fatal）。`step()` 返回 residual/cont_residual/u_min/u_max/u_mean，`_initialize_field`/`residual`/`monitor_payload` 满足 P10 SolverBackend 接口可被直接驱动到 COMPLETED（闭环）。`tests/test_pressure_solver.py` 20 项全绿（残差下降曲线健康 nx=2/3、net decay、全局守恒、内部散度<1e-6、solve_linear exact/numpy/scipy/auto、错误路径、SolverBackend 闭环）；`self_test.py` P5 锚点断言（三路径线性/RHS 维度拒绝/残差下降<1e-5/守恒/进出口闭合/内部散度/诚实拒绝）全过；occ+scdm 两环境 `self_test`+`batch_parse` 21 文件+`run_all` 21 文件全绿无回归 |
| P6 | 湍流族：Spalart-Allmaras → k-ε → k-ω SST → LES 子格子 ✅ 2026-09-06 | 壁面处理/壁面函数 —— **达成**：新增 `turbulence.py`：①壁面处理（`wall_distance` 壁面距离、`y_plus` y+估计、`wall_function` 粘性底层-对数律光滑混合壁面函数、`wall_shear` 壁面剪切应力；`classify_boundaries` 边界分类/`face_volume_flux`/`strain_magnitude` 应变率）；②标量输运（`assemble_scalar_transport` 湍流标量对流-扩散 PDE 装配（∂q/∂t=∇·(ν_eff ∇φ)+prod−diss、边界 ∂φ/∂n=0 / 入口狄氏）+ `solve_scalar` 迭代求解）；③湍流模型族：`SpalartAllmarasSolver`（一方程 ν̃：c_b1 S̃ ν̃ 生成 + c_w1 f_w ν̃²/d² 耗散 + f_v1/f_v2/f_w 阻尼，SA 壁面自适应）、`KEpsilonSolver`（k-ε 两方程：C_μ k²/ε 生成/耗散、σ_k=1/σ_ε=1.3）、`KOmegaSSTSolver`（k-ω SST：F1/F2 混合函数 + CD_kw 交叉扩散 + β*/σ_ω 过渡，ε 壁面自适应）、`LESSmagorinskySolver`（Smagorinsky 子格子 C_s=0.1 + Van Driest 壁面阻尼）；④模型注册表 `make_model` 别名/大小写/连字符不敏感 + `TurbulenceSolver` P10 SolverBackend 兼容门面（step/residual/monitor_payload，nu_t 属性）；⑤**ν_t 耦合到 pressure_solver** —— `PressureSolver(turb_model=...)` 动量装配注入面扩散 `(μ+ρ ν_t)A/d`（内部面+边界面），SIMPLE 环末尾 `_update_turbulence()` 解湍流输运；基线零 ν_t 无回归 + 4 模型集成残差收敛健康（ν_t_max: SA 0.0032 / k-ε 0.0047 / k-ω SST 0.0046 / LES 3e-6）；`tests/test_turbulence.py` 21 项全绿（壁面处理/边界分类/模型注册与别名/各模型 update 残差+nu_t/P10 门面/ν_t 集成四模型）；`self_test.py` P6 锚点断言（壁面处理/模型注册/各模型 update/ν_t 集成/诚实验证）全过；occ+scdm 两环境 `self_test`+`batch_parse` 21 文件+`run_all` 全绿无回归 |
| P7 | 能量/传热（对流扩散+共轭）+ 简化辐射谱系 | ✅ 2026-09-06 | 教程传热工况 —— **达成**：新增 `energy.py` 温度标量输运：①能量方程装配/求解（`assemble_energy_transport` 保守形式 `∇·(ρCp·u·T) − ∇·(κ_eff∇T)=S_T`，κ_eff=κ+κ_t、κ_t=Cp·ρ·ν_t/Pr_t；面电导率 `_face_kappa`；`solve_energy` 欠松弛迭代）；②传热边界全谱系（`BND_ADIABATIC` 绝热零梯度 / `BND_FIXED_TEMP` 定温 / `BND_FIXED_FLUX` 热流密度 / `BND_ROBIN` 第三类对流 `q=h(T_ref−T_w)` / `BND_RADIATION` 简化辐射 `εσT⁴`；边界对流入流源流用比热 `c=cp·mdot` 保能量守恒——修复 cp 漏乘）；③边界分类/辅助（`classify_thermal`/`thermal_btypes`/`volume_to_mass`）；④`EnergySolver`（`set_materials`/`set_radiation_volume`/`set_velocities`/`_effective_kappa`/`_T_boundary`/`update(u,v,w,mdot,nu_t)` 更新温度场返归一化残差，`T` 属性）+ P10 SolverBackend 门面；⑤`ConjugateSolver` 共轭传热（`set_solid_material` 固-液两域、界面连续性由同一稀疏矩阵满足、多次扫掠）；⑥`RadiationModel` Stefan-Boltzmann 简化辐射（`net_emission`/`h_rad`/`volume_source`/`linear_source` 线性化体源）；⑦工厂 `make_energy(model="energy"|"conjugate"|"cht")`；⑧**Boussinesq 浮力耦合** —— `PressureSolver(beta,gravity,buoy_ref_temp,inlet_temp,energy_model=...)` 惰性 `_ensure_energy_model()` 实例化，`_update_energy()` SIMPLE 环尾解能量方程，动量装配注入 `S=−ρ₀β(T−T₀)g·V`；纯 numpy 约束范数走 `solver_run._safe_norm`；`tests/test_energy.py` 17 项全绿（装配/求解有界、边界分类、EnergySolver 收敛/定温壁/材料分区/辐射壁/P10 门面、RadiationModel、工厂、PressureSolver 集成） + `self_test.py` P7 锚点全过；occ+scdm 两环境 `self_test`+`batch_parse` 21 文件+`run_all` 41 文件全绿无回归 |
| P8 | 多相 VOF（几何重构）→ Mixture → DPM 粒子轨 ✅ 2026-09-08 | VOF 几何重构 + Mixture 漂移流 + DPM 拉格朗日—— **达成**：新增 `vof.py`（两相相体积分数 α 守恒输运 + PLIC 几何重构 + 表面张力谱系）：①物性混合 `blend_property`/`two_phase_rho`/`two_phase_mu`（ρ=αρ₁+(1-α)ρ₂、μ 类似，水-空气密度比~850× 单调落在两相间）；②PLIC 几何重构 `plic_normal`（Green-Gauss ∇α + 退化回退）/`plic_plane_offset`（二分）/`keep_phase1_volume`（复用 `mesh_poly.clip_convex` 体积截断 + `poly_volume`，输出夹取 [0,V]）/`plic_reconstruct`（逐单元界面平面满足目标 α）；③守恒输运 `face_upwind_alpha`/`advance_alpha`/`auto_dt`（显式瞬态 CFL 受限一阶上风 + 界面压缩锐化 + [0,1] 有界裁剪 + 守恒缩放，入流边界注入相 1）；④表面张力 `surface_tension_force`（CSF 连续表面力，均匀 α 曲率 0→力≈0）；⑤`VofSolver`（`update(u,v,w,mdot)` 返归一化残差、`rho`/`mu`/`normal`/`offset` 场属性、P10 SolverBackend 门面 `_initialize_field`/`step`/`residual`/`monitor_payload`）+ 工厂 `make_vof`；⑥**单流体耦合** —— `PressureSolver(vof_model="vof")` 惰性 `_ensure_vof_model()` 注入，SIMPLE 环尾 `_update_vof()`，`_face_rho`/`_face_mu` 恒参考密度保持单相矩阵稳定（密度差显式重力体源因水/气密度比过大致 SIMPLE 发散已移除，静水/浮力耦合留待自由面算例 g_rgh 形式）。纯 numpy 约束范数走 `solver_run._safe_norm`。`tests/test_vof.py` 16 项全绿（常量/物性混合/几何重构/守恒输运/表面张力/VofSolver/P10 门面/工厂/PressureSolver 单流体耦合/基线无 α 键）；`self_test.py` P8 锚点断言全过（独立脚本验证，完整 self_test 线性执行至 P1 处 QVTK headless 挂起、属既有问题，P8 段在 P1 之后故以独立运行作为诚实证据）；occ 环境门禁实测无回归：`run_all` 42 个测试文件全绿（含 `test_energy` 17、`test_pressure_solver` 20、`test_mesh_poly` 9、`test_vof` 16 —— 确认移除 VOF 密度差重力源未破坏 P7 Boussinesq 浮力耦合与既有求解器），`batch_parse` 21 个 .sim 全部 OK 写出 `batch_results.json`。**Mixture（多相 drift-flux）** —— 新增 `mixture.py`：①物性混合 `blend_nphase`/`mixture_rho`/`mixture_mu`（ρ_m=Σα_kρ_k、μ_m=Σα_kμ_k，默认 3 相水/气/油）；②Schiller-Naumann 拖曳 `drag_coefficient`（Re<1000 C_D=24/Re(1+0.15Re^0.687)，否则 0.44，re 夹取 EPS 防除零）；③代数滑移 `slip_velocity`（浮力-拖曳平衡）+ 漂移速度 `drift_velocities`（u_dr,k=u_slip,k−Σ_jα_j u_slip,j，保证 Σα_k u_dr,k=0 总体积守恒）+ `volume_flux`；④守恒输运 `face_upwind_alphas`/`advance_mixture`（半隐式有界 CFL 受限一阶上风，单步越界减半时间步重试，返实测 dt_eff）+ `MixtureSolver.auto_dt`；⑤`MixtureSolver`（update 返归一化残差、alphas/rho/mu/slip/drift/phase_volume 场属性、P10 SolverBackend 门面）+ 工厂 `make_mixture`（别名 mixture/mix/drift_flux）；⑥**单流体耦合** —— `PressureSolver(mixture_model="mixture")` 惰性 `_ensure_mixture_model()` + SIMPLE 环尾 `_update_mixture()`，`PressureSolver.rho`/`mu` 属性扩展经 mixture_model 读混合物性；`tests/test_mixture.py` 13 项全绿（常量/物性混合/拖曳系数/滑移/漂移/上风α/advance_mixture/MixtureSolver 初始与 update/P10 门面/工厂/PressureSolver 集成/基线）+ `self_test.py` P8 Mixture 锚点全过。**DPM（离散相粒子）** —— 新增 `dpm.py`：①`locate_cells` 四面体 barycentric 体积坐标定位（域外返 -1）；②半隐式欧拉运动积分 `advance`（dU_p/dt=(U_f−U_p)/τ+g(ρ_p−ρ_f)/ρ_p，拖曳松弛 τ=ρ_p d_p²/(18μ_f)·24/(C_D Re)）+ `auto_dt`；③`inject` 入口注入（parcels/粒径/质量流/初速，`np.where(cid>=0,...)` 修复数组歧义）；④连续相耦合源 `coupling_source`（逐单元力密度 = −Σ m_p (U_f−U_p)/τ / V_c，`np.add.at` 累加到单元）；⑤`DpmSolver`（n_particles/n_active/n_escaped、trajectories、P10 SolverBackend 门面）+ 工厂 `make_dpm`（别名 dpm/discrete_phase/lagrangian）；⑥**单流体耦合** —— `PressureSolver(dpm_model="dpm")` 惰性 `_ensure_dpm_model()` + SIMPLE 环尾 `_update_dpm()`，`_assemble_momentum` 注入 DPM 拖曳反作用力体源；`tests/test_dpm.py` 10 项全绿（常量/点定位/DpmSolver 初始/inject/update/trajectories/P10 门面/工厂/PressureSolver 集成/基线）+ `self_test.py` P8 DPM 锚点全过 |
| P9 | ~~运动谱系：rotating/translating/滑移 interface → morphing → DFBI 6DOF → overset~~（按用户指令顺延）**燃烧算例：组分输运 + 反应动力学** ✅ 2026-09-08 | 教程燃烧工况（methaneOnPt 等）—— **达成**：新增 `species.py`（组分质量分数输运）：①组分换算 `mass_to_mole_frac`/`mole_to_mass_frac`/`mixture_molar_mass`/`gas_density`（X_i=(Y_i/Mw_i)/Σ_j(Y_j/Mw_j)、W_mix=1/Σ_j(Y_j/Mw_j)、ρ=P W_mix/(R T) 理想气体，零分数全零保护免除零）；②单组分对流-扩散质量分数输运 `assemble_species_transport`（∂(ρY)/∂t+∇·(ρuY)=∇·(ρD_eff∇Y)+S，D_eff=D_mol+nu_t/Sc_t）+ `solve_species` 欠松弛迭代；③边界分类 `classify_species`（入口/出口/壁面/边界面全集全覆盖不重叠）+ `volume_mass` 由速度密度重构面质量通量；④`SpeciesSolver`（N-1 输运+补齐组分保证 ΣY=1、set_velocities/set_inlet_fractions/update、Y/w_mix/残差、P10 SolverBackend 门面）+ 工厂 `make_species`（别名 species/multispecies/mass_fraction）。**燃烧反应动力学** —— 新增 `combustion.py`：①全局单步 Arrhenius 反应 `GlobalReaction`（ω=A T^β exp(−Ea/(RT)) Π_i[C_i]^ν_i、`rate` 返 (n_cells,) 数组、`mass_source` 按 Mw_i ν_i 自动满足 ΣS_i=0、`heat_release` 热释放 Q=ω H_rxn）；②火焰诊断 `laminar_flame_speed`/`flame_thickness`（δ_L=α/S_L 幂律）/`combustion_progress`（c=(Y_f0−Y_f)/Y_f0）；③点火器 `make_ignitor`（TemperatureIgnitor/SparkIgnitor（`_safe_norm` 纯 numpy）/ProgressVariableIgnitor 调制反应速率+火花热源）；④`CombustionModel`（内嵌 SpeciesSolver、set_temperature/update(...,T=...)、heat_release/omega/ignited、P10 SolverBackend 门面）+ 工厂 `make_combustion`（别名 combustion/globe/global）。**PressureSolver 耦合** —— `PressureSolver(species_model=...,combustion_model=...,combustion_inlet_fractions=...,combustion_ref_temp=...)` 惰性 `_ensure_species_model()`/`_ensure_combustion_model()`/`_update_species()`/`_update_combustion()` 注入，SIMPLE 环尾推进组分/燃烧源（燃烧单向读能量模型 T 供 Arrhenius 速率），`monitor_payload` 增 sp_y_min/sp_y_max/sp_sum_max/combo_Q_max/combo_T_max/combo_n_ignited；纯 numpy 约束范数走 `solver_run._safe_norm`；`tests/test_combustion.py` 19 项全绿 + `self_test.py` P9 锚点全过，occ 环境 `self_test` 全绿无回归 |
...
| P10-多相 | 多相流算例：欧拉-欧拉（EulerianPhase 族） ✅ 2026-09-08 | 按用户指令「P10 多相流算例」推进（P8 已交付 VOF/Mixture/DPM，欧拉-欧拉补全相间作用力+群体平衡谱系）—— **达成**：新增 `eulerian_multiphase.py`（欧拉-欧拉多相：物性混合 `mixture_rho`/`mixture_mu` 复用 mixture.py + 相间作用力全谱系——拖曳交换 `drag_exchange`（Schiller-Naumann + Wen-Yu 空泡率修正 `f(α_c)=α_c^-2.65`，低 Re 回归 Stokes `18μ_c α_d/d²`）、升力 `lift_force`（Legendre-Magnaudet 简式）、虚拟质量 `virtual_mass_force`（`C_vm α_d ρ_c a_m`，a_m 用对流加速度近似）、壁面润滑 `wall_lubrication_force`（Antal 公式 `C_wl=max(0, Cw1+Cw2 d/y_w)`）；群体平衡 `population_balance_source`（聚并 i→j / 破碎 j→{k<j} / 成核 0→1，Σ_k S_k=0 守恒）+ 核函数 `coalescence_kernel`/`breakup_kernel`/`nucleation_rate`；`EulerianMultiphaseSolver`（N 相 α 守恒、update 返归一化残差、alphas/rho/mu/slip/drift/relative_velocities/phase_velocities/momentum_source/interphase_k 场属性、P10 SolverBackend 门面）+ 工厂 `make_eulerian_multiphase`（别名 eulerian/eulerian_multiphase/ee/euler/euler_euler）；单流体耦合——`PressureSolver(eulerian_model=...)` 惰性 `_ensure_eulerian_model()` + SIMPLE 环尾 `_update_eulerian()`，`rho`/`mu` 属性经 eulerian_model 读混合物性，`monitor_payload` 增 ee_* 键；纯 numpy 约束范数走 `solver_run._safe_norm`；`tests/test_eulerian_multiphase.py` 18 项全绿 + `self_test.py` P10 锚点，occ 环境 self_test/run_all 全绿无回归 |
| P11-运动 | 运动谱系算例：刚体运动+滑移 interface → morphing → DFBI 6DOF → overset + MRF 旋转源 ✅ 2026-09-08 | 按用户指令「P11 算例」推进（原 P9 燃烧顺延的运动谱系，经确认补全）—— **达成**：新增 `motion.py`（运动谱系全链：①刚体运动+滑移 interface——`rotate_points` Rodrigues 旋转（轴上点不动）/`rigid_transform` 刚体变换（平移参考系原点→旋转→平移回+平动位移）/`sliding_interface` 滑移面标识；②morphing 网格变形——`_build_vertex_adjacency` 顶点邻接+边界顶点集 / `morph_mesh`（边界夹持+内部 Laplacian 松弛叠代）；③DFBI 6DOF——`DfbiBody`（mass/inertia/position/quaternion/`_quat_mul`/`_quat_rotate`/`quat_to_dcm` 方向余弦、`world_inertia`/`inv_inertia`/`advance` 半隐式欧拉推进）；④overset 重叠网格——`overset_donor_weights`（最近 k 个 donor 反权重，硬切换退化）/`overset_interpolate`；⑤MRF 旋转参考系源——`mrf_source`（离心 `S=ρω²r_perp` + 科氏 `S=−2ρ Ω×u`，omega=0 归零）；`MotionSolver`（mode 决定更新行为，P10 SolverBackend 门面 `_initialize_field`/`step`/`residual`/`monitor_payload`，`mrf_source(rho)`/`mesh_displacement()`/`body_kinematics()`）+ 工厂 `make_motion`（别名 motion/rigid/rotating/rotation/mrf/sliding/morph/morphing/deform/dfbi/6dof/sixdof/overset/overlapping/chimera，大小写/连字符/下划线不敏感）。**PressureSolver 耦合** —— `PressureSolver(motion_model=...,motion_mode=...,motion_rotation_speed/axis/translation_velocity/reference_point/morph_max_iter/morph_relax/dfbi_dt=...)` 惰性 `_ensure_motion_model()` + SIMPLE 环尾 `_update_motion()` 推进，`_assemble_momentum` 注入 MRF 旋转源（离心+科氏体源×V），`monitor_payload` 增 motion_mode/motion_rotation_speed/motion_disp_max 键，默认 None 零回归；纯 numpy 约束范数走 `solver_run._safe_norm`；`tests/test_motion.py` 22 项全绿 + `self_test.py` P11 锚点，occ 环境 self_test/全量 pytest 486 passed 27 skipped 无回归 |
